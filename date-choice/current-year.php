...
$query = $db->prepare("SELECT amount FROM incomes WHERE user_id = {$_SESSION['logged_id']} AND (date_of_income BETWEEN '{$beginningOfYear->format('Y-m-d')}' AND '{$today->format('Y-m-d')}')");
$query->execute();
...
// Pobieranie wydatków
$query = $db->prepare("SELECT amount FROM expenses WHERE user_id = {$_SESSION['logged_id']} AND (date_of_expense BETWEEN '{$beginningOfYear->format('Y-m-d')}' AND '{$today->format('Y-m-d')}')");
$query->execute();
